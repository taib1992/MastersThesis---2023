...
Af=out.segA(a);
Ef=out.segE(a); % code line added
xf=out.segx(a);
yf=out.segy(a);
a=find(Af>alol);
Af=Af(a);Ef=Ef(a); % code line added
xf=xf(a);
yf=yf(a);
...
b=(xf>a-1);
Af=Af(b);Ef=Ef(b); % code line added
xf=xf(b);
yf=yf(b);
b=(xf<size(Id,1)-a+1);
Af=Af(b);Ef=Ef(b); % code line added
...
out.segA=Af; % code line added
out.segE=Ef; % code line added
out.xoAbsoluteDec=xf; % code line added
out.yoAbsoluteDec=yf; % code line added
...
out.img=zeros(size(Id)); % code line appended to the original code
    ...
    for n=1:numel(out.x)
        for o=1:numel(out.y)
            if(out.xoAbsolute(m)+out.x(n)>0 && out.xoAbsolute(m)+out.x(n)<size(Id,1) && out.yoAbsolute(m)+out.y(o)>0 && out.yoAbsolute(m)+out.y(o)<size(Id,2))
                ...
                out.img(out.xoAbsolute(m)+out.x(n),out.yoAbsolute(m)+out.y(o))=out.d(n,o,m); % code line appended to the original code
            end
        end
    end
end
%figure,imagesc(out.img);axis square;% code line appended to the original code